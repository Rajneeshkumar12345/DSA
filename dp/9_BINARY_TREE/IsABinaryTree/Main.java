...
public class Main {
 ...
 public static pairBST isBST_(Node root){
     ...
     pairBST leftAns = isBST_(root.left);  // left call
     pairBST rightAns = isBST_(root.right); // right Call
     
     pairBST myPair = new pairBST();
    myPair.isBST =  leftAns.isBST && rightAns.isBST && leftAns.max < root.data && root.data < rightAns.min;
    myPair.min = Math.min(root.data, Math.min(leftAns.min, rightAns.min));
    myPair.max = Math.max(root.data, Math.max(leftAns.max, rightAns.max));
    return myPair;
 }
